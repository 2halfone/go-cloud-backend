...
func GenerateQRHandler(c *fiber.Ctx) error {
	...
	userID, _, _, _, err := utils.GetUserFromJWT(c)
	if err != nil {
    ...
    qrJWT, err := services.GenerateQRJWT(eventID, req.EventName, req.Date, userID)
    err = nil
    if err != nil {
    ...
    if err != nil {
      ...
      // Genera immagine QR
    qrImageBase64, err := services.GenerateQRImage(string(qrContentJSON))
    if err != nil {
        return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
    ...
    err = services.CreateAttendanceTable(eventID)
    if err != nil {
...
func ScanQRHandler(c *fiber.Ctx) error {
    ...
    if req.QRContent.JWT == "" || req.Status == "" {
      ...
      // Ottieni user da JWT
    userID, name, surname, role, err := utils.GetUserFromJWT(c)
    if err != nil {
        return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{
            "error": "Errore autenticazione",
        })
    ...
    // Valida JWT del QR
    qrClaims, err := services.ValidateQRJWT(req.QRContent.JWT)
    if err != nil {
        return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
    ...
    if role != "admin" {
        hasScanned, err := services.HasUserScannedEventDynamic(userID, qrClaims.EventID)
        if err != nil {
    ...
    err = services.EnsureUserExists(userID)
    if err != nil {
    ...
    return c.Status(fiber.StatusCreated).JSON(fiber.Map{
        ...
        "status":      "not_registered", // Status is not set automatically, admin can update it later
...
func GetEventAttendanceHandler(c *fiber.Ctx) error {
    ...
    // Verify admin role
    _, _, _, role, err := utils.GetUserFromJWT(c)
    if err != nil {
        return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{
...
func UpdateUserStatusHandler(c *fiber.Ctx) error {
    ...
    adminID, _, _, role, err := utils.GetUserFromJWT(c)
    if err != nil {
        return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{
            "error": "Authentication error",
        })
